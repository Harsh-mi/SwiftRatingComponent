...
///Functions for making Rating view accessible.
struct RatingViewAccessibility {
    
    ///Makes the view accesible by settings its label and using rating as value.
    static func update(_ view: UIView, rating: Double, text: String?, settings: RatingSettings) {
     ...
     */
    static func accessibilityValue(_ view: UIView, rating: Double, settings: RatingSettings) -> String {
     ...
     */
    static func accessibilityIncrement(_ rating: Double, settings: RatingSettings) -> Double {
    ...
    static func accessibilityDecrement(_ rating: Double, settings: RatingSettings) -> Double {
        var increment: Double = 0
...
class RatingText {
    ...
    class func position(_ layer: CALayer, starsSize: CGSize, textMargin: Double) {
        layer.position.x = starsSize.width / 2 - 20
        let yOffset = starsSize.height + 10
        layer.position.y = yOffset
    }
}

 ///Defaults setting values.
struct RatingDefaultSettings {
    init() {}
    ...
    /// When `true` the star fill level is updated when user touches the Rating view. When `false` the Rating view only shows the rating and does not act as the input control.
...
/**
 ...
 Helper class for calculating size for the Rating view.
 
 */
class RatingStarSize {
    /**
     
     Calculates the size of the Rating view. It goes through all the star and text layers and makes size the view size is large enough to show all of them.
...
class RatingLayers {
     ...
     */
    class func createStarLayers(_ rating: Double, settings: RatingSettings, isRightToLeft: Bool) -> [CALayer] {
        ...
        for _ in (0..<settings.totalStars) {
            ...
            let fillLevel = StarRating.starFillLevel(ratingRemainder: ratingRemander,
    ...
    /**
     ...
     - parameter settings: Star view settings.
     - returns: Layer that shows the star. The layer is displayed in the Rating view.
     */
    class func createCompositeStarLayer(_ starFillLevel: Double,
                                        settings: RatingSettings, isRightToLeft: Bool) -> CALayer {
     ...
     */
    class func createPartialStar(_ starFillLevel: Double, settings: RatingSettings, isRightToLeft: Bool) -> CALayer {
    ...
    private class func createStarLayer(_ isFilled: Bool, settings: RatingSettings) -> CALayer {
...
struct RatingTouch {
     ...
     */
    static func touchRating(_ position: CGFloat, settings: RatingSettings) -> Double {
        ...
        rating = StarRating.displayedRatingFromPreciseRating(rating, fillMode: settings.fillMode, totalStars: settings.totalStars)
...
public struct RatingSettings {
    
    /// Returns default set of settings for CosmosView
    public static var `default`: RatingSettings {
    ...
    /// Border color of an empty star.
    public var emptyBorderColor = RatingDefaultSettings.emptyBorderColor
    
    /// Width of the border for empty star.
    public var emptyBorderWidth: Double = RatingDefaultSettings.emptyBorderWidth
    
    /// Border color of a filled star.
    public var filledBorderColor = RatingDefaultSettings.filledBorderColor
    
    /// Width of the border for a filled star.
    public var filledBorderWidth: Double = RatingDefaultSettings.filledBorderWidth
    
    /// Background color of an empty star.
    public var emptyColor = RatingDefaultSettings.emptyColor
    
    /// Background color of a filled star.
    public var filledColor = RatingDefaultSettings.filledColor
     ...
     */
    public var fillMode = RatingDefaultSettings.fillMode
    
    /// Distance between stars.
    public var starMargin: Double = RatingDefaultSettings.starMargin
     ...
     */
    public var starPoints: [CGPoint] = RatingDefaultSettings.starPoints
    
    /// Size of a single star.
    public var starSize: Double = RatingDefaultSettings.starSize
    
    /// The maximum number of stars to be shown.
    public var totalStars = RatingDefaultSettings.totalStars
    ...
    /// Color of the text.
    public var textColor = RatingDefaultSettings.textColor
    
    /// Font for the text.
    public var textFont = RatingDefaultSettings.textFont
    
    /// Distance between the text and the stars.
    public var textMargin: Double = RatingDefaultSettings.textMargin
    ...
    /// The lowest rating that user can set by touching the stars.
    public var minTouchRating: Double = RatingDefaultSettings.minTouchRating
    
    /// Set to `false` if you don't want to pass touches to superview (can be useful in a table view).
    public var passTouchesToSuperview = RatingDefaultSettings.passTouchesToSuperview
    
    /// When `true` the star fill level is updated when user touches the Rating view. When `false` the Rating view only shows the rating and does not act as the input control.
    public var updateOnTouch = RatingDefaultSettings.updateOnTouch
    
    /// Set to `true` if you want to ignore pan gestures (can be useful when presented modally with a `presentationStyle` of `pageSheet` to avoid competing with the dismiss gesture)
    public var disablePanGestures = RatingDefaultSettings.disablePanGestures
}
 ...
 */
@IBDesignable open class RatingStarView: UIView {
     ...
     */
    @IBInspectable open var rating: Double = RatingDefaultSettings.rating {
        didSet {
            if oldValue != rating {
                update()
            }
    ...
    open var settings: RatingSettings = .default {
        didSet {
            update()
        }
    }
     ...
     */
    public convenience init(settings: RatingSettings = .default) {
        self.init(frame: .zero, settings: settings)
    }
    
    /**
     Initializes and returns a newly allocated Rating view object with the specified frame rectangle.
    ...
    public init(frame: CGRect, settings: RatingSettings) {
    ...
    open func update() {
        ...
        var layers = RatingLayers.createStarLayers(
            rating,
            settings: settings,
    ...
    private func createTextLayer(_ text: String, layers: [CALayer]) -> CALayer {
        let textLayer = RatingLayerHelper.createTextLayer(text,
                                                          font: settings.textFont, color: settings.textColor)
        
        let starsSize = RatingStarSize.calculateSizeToFitLayers(layers)
        
        if RightToLeft.isRightToLeft(self) {
            RatingText.position(textLayer, starsSize: CGSize(width: 0, height: starsSize.height), textMargin: 0)
        } else {
            RatingText.position(textLayer, starsSize: starsSize, textMargin: settings.textMargin)
        }
    ...
    private func updateSize(_ layers: [CALayer]) {
        viewSize = RatingStarSize.calculateSizeToFitLayers(layers)
    ...
    /**
     ...
     Prepares the Rating view for reuse in a table view cell.
     If the Rating view is used in a table view cell, call this method after the
    ...
    private func updateAccessibility() {
        RatingViewAccessibility.update(self, rating: rating, text: text, settings: settings)
    }
    ...
    open override func accessibilityIncrement() {
        ...
        rating += RatingViewAccessibility.accessibilityIncrement(rating, settings: settings)
        didTouchRating?(rating)
        didFinishTouchingRating?(rating)
    }
    ...
    open override func accessibilityDecrement() {
        ...
        rating -= RatingViewAccessibility.accessibilityDecrement(rating, settings: settings)
        didTouchRating?(rating)
        didFinishTouchingRating?(rating)
    }
    ...
    /// Closure will be called when user touches the Rating view. The touch rating argument is passed to the closure.
    open var didTouchRating: ((Double)->())?
    
    /// Closure will be called when the user lifts finger from the Rating view. The touch rating argument is passed to the closure.
    open var didFinishTouchingRating: ((Double)->())?
    ...
    func onDidTouch(_ locationX: CGFloat) {
        let calculatedTouchRating = RatingTouch.touchRating(locationX, settings: settings)
        ...
        if calculatedTouchRating == previousRatingForDidTouchCallback {
            // Do not call didTouchRating if rating has not changed
            return
        }
        
        didTouchRating?(calculatedTouchRating)
    ...
    override open func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        let oprimizedBounds = RatingTouchTarget.optimize(bounds)
    ...
    @IBInspectable var totalStars: Int = RatingDefaultSettings.totalStars {
        didSet {
            settings.totalStars = totalStars
        }
    }
    
    @IBInspectable var starSize: Double = RatingDefaultSettings.starSize {
        didSet {
            settings.starSize = starSize
        }
    }
    
    @IBInspectable var filledColor: UIColor = RatingDefaultSettings.filledColor {
        didSet {
            settings.filledColor = filledColor
        }
    }
    
    @IBInspectable var emptyColor: UIColor = RatingDefaultSettings.emptyColor {
        didSet {
            settings.emptyColor = emptyColor
        }
    }
    
    @IBInspectable var emptyBorderColor: UIColor = RatingDefaultSettings.emptyBorderColor {
        didSet {
            settings.emptyBorderColor = emptyBorderColor
        }
    }
    
    @IBInspectable var emptyBorderWidth: Double = RatingDefaultSettings.emptyBorderWidth {
        didSet {
            settings.emptyBorderWidth = emptyBorderWidth
        }
    }
    
    @IBInspectable var filledBorderColor: UIColor = RatingDefaultSettings.filledBorderColor {
        didSet {
            settings.filledBorderColor = filledBorderColor
        }
    }
    
    @IBInspectable var filledBorderWidth: Double = RatingDefaultSettings.filledBorderWidth {
        didSet {
            settings.filledBorderWidth = filledBorderWidth
        }
    }
    
    @IBInspectable var starMargin: Double = RatingDefaultSettings.starMargin {
        didSet {
            settings.starMargin = starMargin
        }
    }
    
    @IBInspectable var fillMode: Int = RatingDefaultSettings.fillMode.rawValue {
        didSet {
            settings.fillMode = StarFillMode(rawValue: fillMode) ?? RatingDefaultSettings.fillMode
        }
    }
    
    @IBInspectable var textSize: Double = RatingDefaultSettings.textSize {
        didSet {
            settings.textFont = settings.textFont.withSize(CGFloat(textSize))
        }
    }
    
    @IBInspectable var textMargin: Double = RatingDefaultSettings.textMargin {
        didSet {
            settings.textMargin = textMargin
        }
    }
    
    @IBInspectable var textColor: UIColor = RatingDefaultSettings.textColor {
        didSet {
            settings.textColor = textColor
        }
    }
    
    @IBInspectable var updateOnTouch: Bool = RatingDefaultSettings.updateOnTouch {
        didSet {
            settings.updateOnTouch = updateOnTouch
        }
    }
    
    @IBInspectable var minTouchRating: Double = RatingDefaultSettings.minTouchRating {
        didSet {
